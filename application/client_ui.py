...
class ClientUi:
    """
    ClientUi renders the Streamlit interface for the decentralized voting app.
    Provides functions to view blockchain, tally votes, and cast votes.
    """

    def __init__(self):
        """Initialize the ClientUi."""
        pass

    def run_ui(self):
        """
        Set up the main user interface for the voting app.
        Displays peer info, blockchain view, voting form, and developer options.
        """
        st.set_page_config(
        ...
        with st.container():
            ...
            with col1:
                st.markdown("""
                    Voters can submit ballots securely through a peer-to-peer (P2P) blockchain network.
                    Each vote is recorded as a block, mined, added to the local blockchain, and broadcast
                    to all peers.
                """)
            with col2:
                st.markdown("""
                    ...
                    <ul style="list-style-type: none; padding-left: 0;">
                        <li><span class="material-icons" style="vertical-align: middle;">ballot</span>
                        Cast your vote securely and anonymously.</li>
                        <li><span class="material-icons" style="vertical-align: middle;">polyline</span>
                        View the local blockchain of votes.</li>
                        <li><span class="material-icons" style="vertical-align: middle;">query_stats</span>
                        See real-time aggregated voting results.</li>
                    </ul>
    ...
    def display_total_votes(self):
        """
        Display the vote tally in a horizontal bar chart.
    ...
    def display_blockchain(self):
        """
        Display the local blockchain visually as connected blocks.
        Refreshes every 0.5 seconds.
        """
        ...
        with st.container(border=True):
            ...
            for i, block_data in enumerate(blockchain_data):
                ...
                if i < len(blockchain_data) - 1:
                    blockchain_html += """
<div style='flex: 0 0 auto; width: 20px; height: 2.5px; background-color: #ccc;
             margin: 0 0.5rem; align-self: center;'></div>"""
    ...
    def get_block_html(self, block, prev_hash_color, hash_color):
        """
        Generate HTML to display a single block.

        Args:
            block (dict): Block data dictionary.
            prev_hash_color (str): Color for previous hash display.
            hash_color (str): Color for current hash display.
        ...
        if transactions != []:
            tx_data = transactions[0]  # Only one transaction per block
            transaction_html = f"""
<p><b>Voter ID:</b> {tx_data.get('voter_id', '')}</p>
<p><b>Candidate ID:</b> {tx_data.get('candidate_id', '')}</p>
<p><b>Transaction timestamp:</b> {tx_data.get('timestamp', '')}</p>"""

        return f"""
<div style='flex: 0 0 auto; width: 250px; border: 0; padding: 1rem; border-radius: 12.5px;
    ...
    <details>
        ...
        <div style='padding-left: 10px;'>{transaction_html}</div>
    </details>
    ...
    def display_voting_options(self):
        """
        Display the voting options form to cast a vote.
        """
        options = st.session_state['client'].ballot_options
